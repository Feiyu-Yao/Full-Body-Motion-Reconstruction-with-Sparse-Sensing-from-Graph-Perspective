...
class Interactor_interactive(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        d = x1.mul(torch.exp(self.phi(x2)))
        c = x2.mul(torch.exp(self.psi(x1)))

        x_even_update = c + self.U(d)
        x_odd_update = d - self.P(c)

        return x_odd_update.permute(0, 2, 1), x_even_update.permute(0, 2, 1)
...
class SCINet_human_structure(nn.Module):
    def __init__(self, in_planes, current_level, kernel_size, dropout, groups, hidden_size, INN):
        ...
        # in_planes = 8
        # self.interact_1 = Interactor_generate(in_planes= in_planes, kernel = kernel_size, dropout = dropout, groups =groups , hidden_size = hidden_size, INN = INN)
    ...
    def forward(self, all_joint):
        ...
        # middle = [0,6,9,15]
        all_joint_new = torch.zeros(all_joint.shape)

        # expanded_joint = self.expand(x.permute(0,2,1)).permute(0,2,1)
        # all_joint = torch.cat([expanded_joint[:,0:15,:],x[:,0:1,:],expanded_joint[:,15:,:], x[:,1:,:]],dim=1)
        # all_joint_new = torch.zeros_like(all_joint)
        # all_joint_new[:,[0,3,6,9],:]= self.interact_1(all_joint[:,[0,3,6,9],:], all_joint[:,[15,20,21],:],index = 1).permute(0,2,1)
        # all_joint_new[:,[1,2,4,5,12,13,14],:]= self.interact_1(all_joint[:,[1,2,4,5,12,13,14],:], all_joint[:,[0,3,6,9,15,20,21],:]).permute(0,2,1)
        # all_joint_new[:,[7, 8, 10, 11, 16, 17, 18, 19],:]= self.interact_1(all_joint[:,[7, 8, 10, 11, 16, 17, 18, 19],:], all_joint[:,[0, 1, 2, 3, 4, 5, 6, 9, 12, 13, 14, 15, 20, 21],:],index= 3).permute(0,2,1)
        all_joint_new[:,0:24,:], all_joint_new[:,24:48,:]= self.interact_1(all_joint[:,0:24,:], all_joint[:,24:48,:])
        # all_joint_new[:,0:29:2,:], all_joint_new[:,1:30:2,:]= self.interact_1(all_joint[:,0:15,:], all_joint[:,15:30,:])

        return all_joint_new
...
class interactive_angle_and_position(nn.Module):
    def __init__(self, output_len, input_len, input_dim = 9, hid_size = 1, num_stacks = 1,
        ...
        super(interactive_angle_and_position, self).__init__()
