...
class SCINet_human_structure(nn.Module):
    ...
    def forward(self, all_joint):
        ...
        # middle = [0,6,9,15]
        all_joint_new = torch.zeros(all_joint.shape).cuda()
        ...
        all_joint_new[:,[0,1,2,3,4,5,6,9,12,13,14],:], all_joint_new[:,[7, 8, 10, 11, 15, 16, 17, 18, 19, 20, 21],:]= self.interact_1(all_joint[:,[0,1,2,3,4,5,6,9,12,13,14],:], all_joint[:,[7, 8, 10, 11, 15, 16, 17, 18, 19, 20, 21],:])
...
class interactive_nodes(nn.Module):
    def __init__(self, output_len, input_len, input_dim = 9, hid_size = 1, num_stacks = 1,
        ...
        super(interactive_nodes, self).__init__()
