...
def convert_to_rgb(minval, maxval, val):
    ...
    i_f = float(val-minval) / float(maxval-minval)
    ...
    if f < EPSILON:
        ...
        if i == 1:
          ...
          # the following one.
        (r1, g1, b1), (r2, g2, b2) = (0,0,255), (255,0,0)
...
def save_animation(body_pose, savepath, bm, fps = 60, resolution = (800,800), gt = None):
    imw, imh = resolution
    ...
    for fId in range(body_pose.v.shape[0]):#
        # body_mesh = trimesh.Trimesh(vertices=c2c(body_pose.v[fId]), faces=faces, vertex_colors=np.concatenate([np.tile(colors['blue'], (445, 1)),np.tile(colors['red'], (6445, 1))]))
        if gt ==None:
            body_mesh = trimesh.Trimesh(vertices=c2c(body_pose.v[fId]), faces=faces)
        else: 
            vertex_colors = np.zeros([body_pose.v[fId].shape[0],3])
            errors = np.sum(np.abs(c2c(body_pose.v[fId]) - c2c(gt.v[fId])),-1)
            max_error = np.max(errors)
            max_l.append(max_error)
            min_error = np.min(errors)
            for i in range(0, body_pose.v[fId].shape[0]):
                (r, g, b) = convert_to_rgb(0, 1.2, errors[i])
                vertex_colors[i][0] = r
                vertex_colors[i][1] = g
                vertex_colors[i][2] = b

            body_mesh = trimesh.Trimesh(vertices=c2c(body_pose.v[fId]), faces=faces, vertex_colors=vertex_colors)
        ...
        body_mesh.apply_transform(trimesh.transformations.rotation_matrix(3.14, (0, 0, 10)))
